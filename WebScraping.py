...
player_list = []
for c in chain(character_row_1, character_row_2):
    character = c.findAll("td", {"class": "rank"})
    rank = character[0].text.strip()
    if rank == 'Rank 3' or rank == 'Rank 2' or rank == 'Rank 1' or rank == 'Guild Master':
        name = c.findAll("td", {"class": "name"})
...
for player in player_list:
    ...
    raider = player
    char_url = ('https://www.wowprogress.com/character/eu/draenor/')
    char_url += raider

    uClient = requests.get(char_url)
